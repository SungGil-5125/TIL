class NextBeautifulNumber {
    fun nextBeautifulNumber(n: Int): Int {
        ...
        while (true) {
            ...
            for (s in str) {
                var count = map.getOrDefault(s, 0)
                map[s] = ++count
            }

            val isBeautifulNumber = map.keys.all { it.digitToInt() == map[it] }
            if (isBeautifulNumber && n < digit) {
                return str.toInt()
            } else {
                digit++
            }
 ...
 * 풀이 후 다시 작성 한 코드
 *
 * 이렇게 풀 수도 있다.. 제약조건을 보고 풀어야함
 */
class Solution {
    fun nextBeautifulNumber(n: Int): Int {
        val beautifulNumbers = intArrayOf(
        ...
        return beautifulNumbers.first { it > n }
    }
}
